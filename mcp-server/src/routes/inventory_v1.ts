...
const manifestCheckoutSchema = z.object({ client_id: z.string().uuid(), start_date: z.string().date(), end_date: z.string().date(), deposit_amount: z.number().nonnegative().default(0), deposit_paid: z.boolean().default(false), notes: z.string().optional(), barcodes: z.array(z.string().trim().min(1).max(200)).min(1) }).strict();
...
export function registerInventoryV1Routes(app: Express, auth: ReturnType<typeof import("../middleware/supabase_auth.js").createSupabaseAuthMiddleware>, database: SupabaseRestClient): void {
  ...
  app.get("/api/v1/rentals/:id/scanned-items",auth,async(req,res)=>{try{res.json(ok({items:await database.get(`rental_scan_items?rental_id=eq.${encodeURIComponent(String(req.params.id))}&select=*&order=barcode.asc`)}));}catch(error){handleError(res,error);}});
  ...
  app.post("/api/v1/rentals/manifest-checkout", auth, async (req: SupabaseAuthenticatedRequest, res) => { const parsed=manifestCheckoutSchema.safeParse(req.body); const requestId=String(req.header("idempotency-key")??""); if(!parsed.success || !req.user || !isUuid(requestId)){res.status(422).json(fail("VALIDATION_ERROR","Invalid scan checkout payload"));return;} try {res.status(201).json(ok({rental:await database.rpc("create_manifest_checkout_rental",{p_client_id:parsed.data.client_id,p_created_by:req.user.id,p_start_date:parsed.data.start_date,p_end_date:parsed.data.end_date,p_deposit_amount:parsed.data.deposit_amount,p_deposit_paid:parsed.data.deposit_paid,p_notes:parsed.data.notes??"",p_barcodes:parsed.data.barcodes,p_request_id:requestId})}));}catch(error){handleError(res,error);} });
  ...
  app.post("/api/v1/rentals/:id/return", auth, async (req: SupabaseAuthenticatedRequest,res)=>{
    ...
    if (!req.user || !isUuid(requestId) || !isUuidArray(verifiedRentalItemIds) || !Array.isArray(returnedQuantities)) {
      res.status(422).json(fail("VALIDATION_ERROR", "A request ID, scanned rental item IDs, and return quantities are required"));
      return;
    }
    try { res.json(ok({rental:await database.rpc("confirm_return",{p_rental_id:req.params.id,p_created_by:req.user.id,p_verified_rental_item_ids:verifiedRentalItemIds,p_returned_quantities:returnedQuantities,p_request_id:requestId})})); }
    catch(error){handleError(res,error);}
  });
  app.post("/api/v1/rentals/:id/manifest-return", auth, async (req: SupabaseAuthenticatedRequest,res)=>{ const requestId=String(req.header("idempotency-key")??""); const barcodes=req.body?.barcodes; if(!req.user || !isUuid(requestId) || !Array.isArray(barcodes) || !barcodes.every((barcode)=>typeof barcode === "string" && barcode.trim().length > 0)){res.status(422).json(fail("VALIDATION_ERROR","A request ID and returned barcodes are required"));return;} try {res.json(ok(await database.rpc("confirm_manifest_return",{p_rental_id:req.params.id,p_created_by:req.user.id,p_barcodes:barcodes,p_request_id:requestId})));}catch(error){handleError(res,error);} });
}
