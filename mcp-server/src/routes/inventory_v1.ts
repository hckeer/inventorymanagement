...
export function registerInventoryV1Routes(app: Express, auth: ReturnType<typeof import("../middleware/supabase_auth.js").createSupabaseAuthMiddleware>, database: SupabaseRestClient): void {
  ...
  app.get("/api/v1/products/:id/checkout-suggestions", auth, async (req,res)=>{try{
    ...
    if (suggestions.length===0) { res.json(ok({ suggestions: [] })); return; }
    const ids=suggestions.map((suggestion)=>suggestion.quantity_product_id).join(",");
    const products=await database.get<Array<Record<string, unknown>>>(`products?id=in.(${encodeURIComponent(ids)})&select=*,stock_balances(*)`);
    const productsById=new Map(products.map((product)=>[String(product.id),product]));
    res.json(ok({ suggestions: suggestions.map((suggestion)=>({ ...suggestion, product: productsById.get(suggestion.quantity_product_id) })).filter((suggestion)=>suggestion.product != null) }));
